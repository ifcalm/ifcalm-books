#!/usr/bin/env python3
"""Generate the core Daji collection from CBETA's stable juan endpoint."""
...
CONFIG = {
    "work": "T0397",
    "display_title": "大方等大集经",
    "tag": "大方等大集经",
    "slug": "da-fang-deng-da-ji-jing",
    "target": ROOT / "content/posts/buddha/jingzang/daji/da-fang-deng-da-ji-jing",
    "total_juan": 60,
    "weight": 10,
    "summary": "《大方等大集经》共六十卷。",
    "index_intro": "收录《大方等大集经》共六十卷。",
}

TITLE_PATTERN = re.compile(r"^大方等大集經卷第[零一二三四五六七八九十百]+$")
PRESERVE_BYLINE_JUANS = {1, 38, 46}
...
def drop_repeated_title_and_nonsegment_bylines(blocks, juan: int):
    cleaned = []
    preserve_byline = juan in PRESERVE_BYLINE_JUANS
    pending_byline = None
    removed_title_before = False

    for block_type, level, text in blocks:
        normalized = normalize_text(text)
        if block_type == "paragraph" and TITLE_PATTERN.fullmatch(normalized):
            removed_title_before = True
            continue
        if preserve_byline and block_type == "byline" and removed_title_before:
        ...
        if pending_byline is not None and block_type == "head":
            ...
            removed_title_before = False
            continue
        if block_type == "byline" and not preserve_byline:
            continue
        cleaned.append((block_type, level, text))
        removed_title_before = False
...
def fetch_blocks(juan: int):
    ...
    return drop_repeated_title_and_nonsegment_bylines(parser.blocks, juan)
